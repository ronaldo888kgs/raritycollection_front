...
export default function LoginPage() {
  ...
  const submitLogin = async () => {
    ...
    if(!emailValidate(username)) return;
    ...
    if(activation_code !== '')
      history.push({ pathname: auth? '/email_verify/' + auth : '/email_verify', state: activation_code});
  }
    ...
    <DefaultPageTemplate>
      <S.Container>
        <S.LoginBox> 
          ...
          <div>
            <div>
              <S.Input maxLength={60} value={username} placeholder="Enter Email" onChange={(e: any) => setUsername(e.target.value)} onKeyPress={(e:any) => handleKeyPress(e, 'password')} />
            </div>
            <div>
